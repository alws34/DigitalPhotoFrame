            ...
            try:
                ...
                with get_db() as conn:
                    ...
                    for img_hash, img in data.items():
                        ...
                        """,
                            (
                                img.get("hash", img_hash),
                                img.get("absolute_path"),
                                img.get("relative_path"),
                                img.get("filename"),
                                img.get("caption"),
                                img.get("date_added"),
                                img.get("date_modified"),
                                img.get("date_taken"),
                                img.get("filesize"),
                                img.get("height"),
                                img.get("width"),
                                img.get("last_displayed"),
                                img.get("uploader"),
                                img.get("views", 0),
                            ),
                        )
                print(
                    f"[Database] Migrated {len(data)} image metadata records from {metadata_json_path}"
                )
...
def update_user_login(uid, current_time):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE users SET last_login = ?, failed_count = 0 WHERE uid = ?",
            (current_time, uid),
        )
...
def increment_failed_login(username):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE users SET failed_count = failed_count + 1 WHERE username = ?",
            (username,),
        )
...
def lock_user(username, lock_until):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE users SET lock_until = ? WHERE username = ?", (lock_until, username)
        )
...
def update_password_db(uid, pw_hash, algo, password_changed_at):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE users SET pw_hash = ?, algo = ?, password_changed_at = ? WHERE uid = ?",
            (pw_hash, algo, password_changed_at, uid),
        )


def create_user_db(
    uid, username, email, pw_hash, role, algo, created_at, password_changed_at
):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
...
# Whitelist of columns allowed in images_metadata writes. Keys arriving from
# request payloads are filtered against this set so they can never be
# interpolated as arbitrary SQL identifiers.
_IMAGE_METADATA_COLUMNS = frozenset(
    {
        "hash",
        "absolute_path",
        "relative_path",
...
def update_metadata(img_hash, data):
    # Only persist recognized columns. Any unexpected key (potential SQL
    # identifier injection or schema drift) is dropped and logged.
    safe_data = {}
    for k, v in data.items():
        if k in _IMAGE_METADATA_COLUMNS:
            safe_data[k] = v
        else:
            print(f"[Database] update_metadata: ignoring unknown column '{k}'")
    ...
    with get_db() as conn:
        ...
        if cursor.fetchone():
            ...
            cursor.execute(
                f"UPDATE images_metadata SET {', '.join(fields)} WHERE hash = ?", values
            )
        else:
            # Insert
            safe_data["hash"] = img_hash
            keys = ", ".join(safe_data.keys())
            placeholders = ", ".join(["?"] * len(safe_data))
            cursor.execute(
                f"INSERT INTO images_metadata ({keys}) VALUES ({placeholders})",
